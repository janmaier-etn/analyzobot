...
export class AIAnalyzer {
     ...
     * @param {Array} annualReports - Annual reports data
     * @returns {Promise<Object>} Analysis results
     */
    async analyzeCompany(companyData, annualReports = []) {
        try {
            const prompt = this.buildAnalysisPrompt(companyData, annualReports);
     ...
     * @param {Array} annualReports - Annual reports
     * @returns {string} Prompt
     */
    buildAnalysisPrompt(companyData, annualReports) {
        const hasReports = annualReports && annualReports.length > 0;
...
${hasReports ? `- Počet dostupných výročních zpráv: ${annualReports.length}` : '- Výroční zprávy nejsou k dispozici'}
