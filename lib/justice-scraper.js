...
export class JusticeScraper {
    ...
    async getReportsSummary(ico) {
        ...
        // If no reports found, create a direct link to justice.cz
        if (reports.length === 0) {
            console.log('⚠️ No reports found, creating direct justice.cz link');
            const currentYear = new Date().getFullYear();

            reports = [
                {
                    title: 'Zobrazit výroční zprávy na Justice.cz',
