...
export class JusticeScraper {
    ...
    async findCompanyUrl(ico) {
        try {
            const searchUrl = `${JUSTICE_BASE_URL}/rejstrik-$firma?ico=${ico}&jenPlatne=PLATNE&polozek=50&typHledani=STARTS_WITH`;
    ...
    async getReportsSummary(ico) {
        ...
        // If no reports found, create a direct link to search portals
        if (reports.length === 0) {
            console.log('⚠️ No reports found, creating search links');
            const currentYear = new Date().getFullYear();

            reports = [
                {
                    title: 'Vyhledat výroční zprávy na Justice.cz',
                    url: `https://or.justice.cz/ias/ui/rejstrik-$firma?ico=${ico}&jenPlatne=PLATNE&polozek=50&typHledani=STARTS_WITH`,
                    year: currentYear,
                    type: 'justice_link'
                },
                {
                    title: 'Zobrazit firmu na Firmy.cz (výroční zprávy v záložce Ekonomika)',
                    url: `https://www.firmy.cz/detail/${ico}`,
                    year: currentYear,
                    type: 'firmy_link'
                }
            ];
